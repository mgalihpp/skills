...
- Any code → name the data shape first, and choose its organizing structure per **principle-model-the-domain** skill.
...
## Subagents — universal (Cursor / opencode / oh-my-pi)

**Use `poteto-agent` for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers).
- **Cursor:** `Task` with `subagent_type: "poteto-agent"`
- **opencode / oh-my-pi / Claude Code:** `task` with `agent: "poteto-agent"` (or `agent://poteto-agent`) — same markdown definition at `.agents/agents/poteto-agent.md` / `.opencode/agents/` / `.claude/agents/`
- **OMP native:** `agent("...prompt...", { agent: "poteto-agent" })` — discovered via `.agents/skills` + `.agents/agents`

`/poteto-mode` and `poteto-agent` route through the same wrapper. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own subagent type for diverse-model review; respect what the skill prescribes (`generalPurpose` / `poteto-agent` / `comment-sicko`), don't override to `poteto-agent`. Translation:
- **Cursor:** `subagent_type: "generalPurpose"` / `"Comment Sicko"` / `"poteto-agent"`
- **opencode/OMP:** `agent: "generalPurpose"` / `agent: "comment-sicko"` / `agent: "poteto-agent"` (lowercase hyphen)
